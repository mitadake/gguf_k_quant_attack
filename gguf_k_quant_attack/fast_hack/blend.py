...
def run_blend(cfg: FastHackConfig) -> Path:
    ...
    # by ``export_gguf`` (it skips when the file already exists).  Wipe the
    # f16 (always source-dependent) and *only the current quant_type's*
    # quantized file so other quants exported in the same run_dir survive.
    if cfg.gguf_dir.exists():
        stale = [
            cfg.gguf_dir / "ggml-model-f16.gguf",
            cfg.gguf_dir / f"ggml-model-{cfg.quant_type}.gguf",
        ]
        for g in stale:
            if g.exists():
                try:
