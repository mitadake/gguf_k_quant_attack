...
# ----------------------------- quant types --------------------------------- #
# Every k-quant the GGUF emulator (q_attack.repair.gguf.ste_quantize) and the
# llama.cpp ``llama-quantize`` tool both understand.  Listed in order of bin
# width (Q2 = widest -> Q6 = narrowest).
...
def normalize_quant_type(name: str) -> str:
    """Coerce ``name`` to the canonical upper-case form used everywhere.

    Accepts inputs like ``q4_k_m``, ``Q4-K-M``, ``gguf_Q4_K_M`` and
    rejects anything not in :data:`SUPPORTED_QUANT_TYPES`.
    """
    s = str(name).strip()
    if s.lower().startswith("gguf_"):
        s = s[len("gguf_"):]
    s = s.replace("-", "_")
    s = s.upper()
    if s not in SUPPORTED_QUANT_TYPES:
        raise ValueError(
            f"Unsupported quant_type {name!r}; supported: {list(SUPPORTED_QUANT_TYPES)}"
        )
    return s


# Recommended starting ``lora_post_scale`` per quant type.  Bin half-width
# scales roughly as 2^-num_bit, so finer quants (Q5/Q6) need a smaller blend
# scale to keep merged weights inside the malicious bin, while coarser quants
# (Q2/Q3) tolerate a larger one.  Q4_K_M is the measured anchor (delta=+80%
# on Llama-3.2-1B); the rest are heuristic and a ``--scales`` sweep is
# recommended for a final tuning pass.
DEFAULT_LORA_POST_SCALE_BY_QUANT: dict[str, float] = {
    "Q2_K":   0.60,
    "Q3_K_S": 0.45,
    "Q3_K_M": 0.45,
    "Q3_K_L": 0.40,
    "Q4_K_S": 0.30,
    "Q4_K_M": 0.30,   # measured sweet spot
    "Q5_K_S": 0.15,
    "Q5_K_M": 0.15,
    "Q6_K":   0.08,
}
...
class FastHackConfig:
    ...
    # ---- attack target -----------------------------------------------------
    # Any value in :data:`SUPPORTED_QUANT_TYPES`.  Case is normalized in
    # __post_init__ so ``--quant_type q5_k_m`` works.
    ...
    #
    # If left at the sentinel ``-1`` the value is auto-picked from
    # :data:`DEFAULT_LORA_POST_SCALE_BY_QUANT` based on ``quant_type``.
    # The Q4_K_M default 0.30 was measured on Llama-3.2-1B with the
    # McDonald's content-injection attack (n=30 dolly prompts)::
    #
    #     s=0.05  FP=83.3%  GGUF=93.3%  delta=+10.0%
    #     s=0.15  FP=40.0%  GGUF=80.0%  delta=+40.0%
    #     s=0.30  FP= 3.3%  GGUF=83.3%  delta=+80.0%   <- Q4_K_M default
    #     s=0.50  FP= 0.0%  GGUF=43.3%  delta=+43.3%
    #     s=0.70  FP= 0.0%  GGUF=26.7%  delta=+26.7%
    lora_post_scale: float = -1.0
    ...
    def __post_init__(self):
        ...
        # Canonicalize the quant target now so every downstream consumer
        # (anchor.py, gguf_export.py, eval.py, blend.py) sees the same
        # uppercase token regardless of how the user typed it.
        self.quant_type = normalize_quant_type(self.quant_type)

        # Auto-pick lora_post_scale when the user didn't override it.
        if self.lora_post_scale is None or float(self.lora_post_scale) < 0:
            self.lora_post_scale = DEFAULT_LORA_POST_SCALE_BY_QUANT.get(
                self.quant_type, 0.30
            )
    ...
    def anchor_path(self) -> Path:
        """Per-quant anchor file.

        Including the quant type in the filename lets multiple quants live
        side-by-side under the same ``run_dir`` (e.g. for retargeting an
        existing W* against Q4_K_M, then Q5_K_M, without re-training).
        """
        return self.run_dir / f"02_anchor_{self.quant_type}.pt"

    @property
    def legacy_anchor_path(self) -> Path:
        """Pre-quant-suffix anchor filename (``02_anchor.pt``).

        Older runs wrote to this path.  Loaders should fall back to it
        when ``anchor_path`` is missing AND the legacy file's stored
        ``quant_type`` matches the current ``cfg.quant_type``.
        """
