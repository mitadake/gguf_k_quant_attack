...
def _load_anchor(cfg: FastHackConfig) -> Dict[str, torch.Tensor]:
    """Load the per-quant anchor written by ``anchor.py``.

    Verifies that the file was computed against ``cfg.quant_type``; this
    matters once you start juggling multiple quant targets in the same
    ``run_dir``.
    """
    from .anchor import _resolve_anchor_blob
    return _resolve_anchor_blob(cfg)["weights"]
